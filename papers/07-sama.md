...
- `projects/llava_sam2/models/internvl_sama.py`：**核心模型 `InternVL_Slowfast`**，含时空上下文聚合器（:346-617）。
...
**(a) 被框区域 → referent token（时空上下文聚合器）**，`spatial_temporal_token_generation`（`internvl_sama.py:346-565`）：
1. **Spatial Q-Former**：InternViT 帧特征经 `vlm2spatial_Qformer_proj`→1408 维，与 32 个 query token 做 cross-attn，每帧压成 32 token（:481-495）。
2. **Mask-Pooling 区域特征**：对每个被框目标用 `MaskPooling` 在帧特征上按 mask 池化（`Qformer_mask_pooling_proj_st`），并把目标框下采样到 112×112 经 MLP 编码（`Qformer_mask_proj_st`，:417-435）。两路经 `<mask>→<vp><mask><pos></vp>` 注入 **Temporal Q-Former** 的 `<mask>`(id 30524)/`<pos>`(id 30523) 位置（`qformer.py:126-141`）。
3. **Temporal Q-Former**（2 层、32 query）跨帧聚合，按 `window_size=512` 分窗（:519-544）。
4. **`temporal_context_inject`**（:567-617）：聚合的 temporal token 作 K/V、原帧视觉特征作 Q 做 attention，时间维取 mean，得最终 **referent token**，splice 进 LLM 输入 embedding 的 `<vp>...</vp>` 位置（:786-857）。
...
3. **SAMA-239K 数据集 + 自动标注流水线**：复用 5 个 VOS/grounding 数据集 mask，借多模态模型按"彩框+`<objN>`标签"自动生成 grounded 多轮 QA，规模化产出 15K 视频/17 万 QA，并配套 SAMA-Bench。
