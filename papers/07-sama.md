...
2. **Mask-Pooling 区域特征**：对每个被框目标用 `MaskPooling` 在帧特征上按 mask 池化（`Qformer_mask_pooling_proj_st`），并把目标框下采样到 112×112 经 MLP 编码（`Qformer_mask_proj_st`，:417-435）。两路经 `<mask>→<vp><mask><pos></vp>` 注入 **Temporal Q-Former** 的 `<mask>`(id 30523, `VISUAL_TOKEN_ID`, 注入 mask-pooling 视觉特征)/`<pos>`(id 30524, `MASK_TOKEN_ID`, 注入 112×112 框形状特征) 位置（`qformer.py:72-73, 128-141`）。
3. **Temporal Q-Former**（2 层、32 query）跨帧聚合，按 `window_size=512` 分窗（:519-544）。
4. **`temporal_context_inject`**（:553-595）：聚合的 temporal token 作 K/V、原帧视觉特征作 Q 做 attention，时间维取 mean，得最终 **referent token**，splice 进 LLM 输入 embedding 的 `<vp>...</vp>` 位置（:786-857）。
