...
3. **贝叶斯解释 —— Visual Thinking Drift 的六步推导**（paper.txt:123-152，论文 §3.2）：
   - **(1) 生成过程分解**：把"思维链 + 答案"的联合生成写成自回归乘积 `p(c_{1:T}, a | q, v) = p(a | c_{1:T}, q, v) · ∏_{t=1}^{T} p(c_t | c_{<t}, q, v)`——答案条件于整条链，每个思维 token `c_t` 又条件于已生成前缀、问题与视频。
   - **(2) 单步 softmax 的双项结构**：每步的 logit 可拆成两个加性来源——**语言先验项 `hᵀ·W_lang`**（来自纯文本上下文 `c_{<t}, q`）与**视觉似然项 `h_vᵀ·W_vis`**（来自视频 `v`）。token 由二者之和经 softmax 决定。
   - **(3) 权重范数失衡**：实证上 **‖W_lang‖ ≫ ‖W_vis‖**——语言通路的权重范数远大于视觉通路，故在 logit 竞争中语言先验天然占优，视觉证据需要很强才能扭转。
   - **(4) 视觉似然随链长被稀释**：随着思维链 token 数 `t` 增大，自注意力里视频 token 占比被越来越多的文本 token 摊薄，`h_vᵀ·W_vis` 的相对影响单调下降——链越长，模型越"看不见"视频。
   - **(5) 失败率随链长线性增长**：设单步不被语言先验带偏（视觉正确）的概率为 `1−ε`，则长度 `T` 的链全程视觉忠实的概率 `(1−ε)^T ≈ 1−Tε`（一阶近似），即**整链失败率 ≈ Tε，随链长线性放大**。这解释了"CoT 越长反而越差"。
   - **(6) 后验塌缩到幻觉**：自回归无回溯，一旦早期某 token 锁定了一个**不存在的视觉事实**，后续所有 token 都以它为条件，后验 `p(a|c_{1:T},·)` 塌缩到这个幻觉分支——逻辑自洽却脱离视频。VER 的对策正是用奖励把视觉似然"显式拉回"到每条被采样的链上（见下文药方阶段）。
