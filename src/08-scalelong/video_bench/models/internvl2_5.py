...
def split_model(model_name):
    model_name = model_name.split('/')[-1].replace('-MPO', '')
    device_map = {}
...
class Internvl2_5(BasicModel):
    ...
    def generate_video_only(self, visual, text,frame_num) -> str:
        generation_config = dict(max_new_tokens=1024, do_sample=False)
        video_path = visual
    
        pixel_values, num_patches_list, pixel_values_list = load_video(
                video_path,
    ...
    def generate_video_only_res(self, visual1, text, target_resolution=None, keep_aspect_ratio = True, min_pixels = None, max_pixels = None) -> str:
        ...
        pixel_vi = []
        video_path = visual1
        pixel_values, num_patches_list, pixel_values_list = load_video_res(
            video_path, num_segments=self.num_segments, max_num=self.max_num, target_resolution=target_resolution, keep_aspect_ratio=keep_aspect_ratio, min_pixels=min_pixels, max_pixels=max_pixels
        )
