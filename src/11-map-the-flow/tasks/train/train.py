...
from tasks.shared_utils import (INTERNVL_IMG_START_TOKEN, INTERNVL_IMG_CONTEXT_TOKEN, INTERNVL_IMG_END_TOKEN,
                                LLAVA_IMAGE_TOKEN, IMAGENET_MEAN, IMAGENET_STD)

from transformers import (DataCollatorWithPadding, get_scheduler, AutoTokenizer,
                          AutoModel, AutoModelForCausalLM, CLIPImageProcessor)
...
def setup_model_internvl(
    ...
    if config.model.torch_dtype in ('bfloat16', 'float16', 'float32'):
        ...
        torch_dtype = config.model.torch_dtype
    logger.info("Creating InternVLChatModel...")

    # 1. Build processor
    # 1.1 Load tokenizer
    logger.info(f'Loading Tokenizer: {config.model.repo_id}')
    tokenizer = AutoTokenizer.from_pretrained(config.model.repo_id, add_eos_token=False,
    ...
    # 2. Load model config
    model_config = InternVLChatConfig.from_pretrained(config.model.repo_id,
                                                      img_context_token_id=img_context_token_id,
                                                      ignore_index=config.preprocess.ignore_index,
                                                      num_frames=config.num_frames
                                                      )
    model_config.vision_config.drop_path_rate = config.model.vision_encoder.drop_path_rate

    # 3. Load model
    model = InternVLChatModel.from_pretrained(config.model.repo_id, torch_dtype=torch_dtype, config=model_config)

    # Manually force uncare token as pad token
    ...
    if config.model.freeze_projector:
        logger.info("freezing parameters in model.mlp1")
        for p in model.mlp1.parameters():
            p.requires_grad = False

    if config.model.freeze_vision_tower:
        logger.info("freezing parameters in model.vision_model")
        for p in model.vision_model.parameters():
            p.requires_grad = False

    # Add Lora
    if config.model.use_lora:
        model.wrap_llm_lora(r=config.model.lora_r, lora_alpha=config.model.lora_alpha,
                            lora_dropout=config.model.lora_dropout)
        model.config.use_llm_lora = config.model.use_lora
...
def preprocess(
    ...
    input_list, images = [], []
    for sample in batch:
        image, tex, instruction, index = sample  # (nframe, 3, 336, 336), (0-255)
        if model_type == 'llava':
            tex = (tex.replace(args.dataset_video_placeholder, LLAVA_IMAGE_TOKEN).
                   replace(args.dataset_image_placeholder, LLAVA_IMAGE_TOKEN))
        elif model_type == 'internvl':
            # <video></video> -> <img><IMG_CONTEXT></img>
            vision_tokens = f'{INTERNVL_IMG_START_TOKEN}{INTERNVL_IMG_CONTEXT_TOKEN}{INTERNVL_IMG_END_TOKEN}'
            tex = tex.replace(args.dataset_video_placeholder, vision_tokens)
            # if args.dataset_image_placeholder in tex: #TODO exception for image placeholder
        else:
            raise NotImplementedError
        seps = [role for role in args.roles]    # ['USER:', 'ASSISTANT:']
...
def main(config):
    ...
    if config.model_type == 'llava':
        setup_model_func = setup_model_llava
    elif config.model_type == 'internvl':
        setup_model_func = setup_model_internvl
    else:
        raise NotImplementedError

    model, processor = setup_model_func(config, find_unused_parameters=True)
    ...
    weight_save_keywords = ["lora_"]
    if config.model_type == "llava":
        weight_save_keywords.append("multi_modal_projector")
        if not config.model.freeze_lm:
            weight_save_keywords.append("language_model")
    elif config.model_type == "internvl":
        weight_save_keywords.append("mlp1")
    else:
        raise NotImplementedError

    "===== Dataloader ====="
    collate_fn = DataCollatorWithPadding(tokenizer=processor.tokenizer, padding='max_length',
                                         max_length=config.max_txt_l, return_tensors='pt')
    collate_fn = functools.partial(preprocess, args=config.preprocess, processor=processor,
                                   collate_fn=collate_fn, model_type=config.model_type)
    ...
    "===== Set up logging ====="
    # Reference: logging with Accelerate https://docs.wandb.ai/guides/integrations/accelerate/
    experiment_config = {
        'num_frames': config.num_frames,
    ...
    if config.report_to == "wandb":
        experiment_config = config  # include all the important hyperparam
        logging_setups['wandb'] = {
            "name": os.path.basename(config.output_dir),
            "dir": config.wandb.dir
        }

    accelerator.init_trackers(project_name=config.project_name, config=experiment_config, init_kwargs=logging_setups)
                ...
                if global_step % config.save_steps == 0:
                    ...
                    with torch.no_grad():
                        ...
                        if not config.deepspeed:
                            save_state_dict = {
                                k: v for k, v in accelerator.get_state_dict(model).items()
                                if any(keyword in k for keyword in weight_save_keywords)
                            }
                        else:
