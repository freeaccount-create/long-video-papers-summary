...
def chatgpt_eval(res, model="gpt-3.5-turbo-0125"):
    ...
    # thread_name = current_process().name  # Get the name of the current process (thread)
    # thread_pid = os.getpid()  # Get the process ID
    # print(f"Evaluating data in thread {thread_name} (PID: {thread_pid})")
...
def save_results(result_list, save_path, model="gpt-3.5-turbo-0125"):
    ...
    num_threads = 2 # Original code was implemented with 7 threads, but this leads to max request limit
    ...
    # result_list = [chatgpt_eval(res, model=model) for res in tqdm.tqdm(result_list)]
...
class VideoQABenchDataset(EvalDataset):
    ...
    data_list_info = OrderedDict({
        ...
        "MSRVTT_QA": OrderedDict(
            ...
            prefix=f"{data_dir}/MSRVTT_Zero_Shot_QA/videos/all",
            data_type="video", 
            bound=False,
            question_key='question',
            answer_key='answer',
        ...
        "ActivityNet": OrderedDict(
            q_json_relpath="ActivityNet_Zero_Shot_QA/test_q.json",
            a_json_relpath="ActivityNet_Zero_Shot_QA/test_a.json",
            prefix=f"{data_dir}/ActivityNet_Zero_Shot_QA/videos",
            data_type="video", 
            bound=False,
            question_key='question',
            answer_key='answer',
        ...
        "TGIF_QA": OrderedDict(
            q_json_relpath="TGIF_Zero_Shot_QA/test_q.json",
            a_json_relpath="TGIF_Zero_Shot_QA/test_a.json",
            prefix=f"{data_dir}/TGIF_Zero_Shot_QA/mp4",
            data_type="video",  # gif
            bound=False,
            question_key='question',
            answer_key='answer',
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k, v in data_list_info.items():
            ...
            for i in indexs:
                ...
                data = {}
                # Ensure that values in question/answer_data are treated as strings
                question_data = {key: str(value) for key, value in question_data.items()}
                answer_data = {key: str(value) for key, value in answer_data.items()}
