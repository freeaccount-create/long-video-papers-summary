...
from openai import RateLimitError  # Import RateLimitError directly
...
def chatgpt_eval(res, model="gpt-3.5-turbo-0125", correctness_only=False):
    pred = res['pred']
    gt = res['gt']
    ...
    if task_type == 'generic_qa':
        ...
        subtasks = ('correctness',) if correctness_only else ('context', 'detailed_orientation', 'correctness')
        for sub_task_type in subtasks:
            if pred=="":
                print("no pred")
                score = 0
            else:
...
def save_results(result_list, save_path, model="gpt-3.5-turbo-0125", correctness_only=False):
    dump_json(result_list, save_path, 'inference_results.json')

    # Original code was implemented with 7 threads, but this leads to max request limit
    # Tier1: maximum 2 threads
    num_threads = 4
    with Pool(num_threads) as pool:
        func = partial(chatgpt_eval, model=model, correctness_only=correctness_only)
...
class VideoChatGPTBenchDataset(EvalDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k, v in data_list_info.items():
            ...
            # e.g.,
            # {'Q1': 'What is the parrot eating from the pink coffee cup on the kitchen counter?',
            #  'Q2': 'On what container is the butterfly painted, that the parrot is eating from on the kitchen counter?'
            #  'A': 'The parrot is eating cream from a pink coffee cup with a butterfly painted on it.',
            #  'video_name': 'v_IN23CO8Kotk'}
