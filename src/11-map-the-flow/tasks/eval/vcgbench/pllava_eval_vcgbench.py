...
def parse_args():
    ...
    parser.add_argument(
        "--correctness_only",   # When GPT eval, consider correctness only in general eval
        action='store_true'
    )
    parser.add_argument(
        "--pooling_shape", 
...
def load_model_and_dataset(rank, world_size, pretrained_model_name_or_path, num_frames, use_lora, lora_alpha,
                           weight_dir, test_ratio, pooling_shape=(16,12,12), use_multi_gpus=False,
...
def main():
    ...
    save_results(result_list, save_path, model=eval_model, correctness_only=args.correctness_only)
