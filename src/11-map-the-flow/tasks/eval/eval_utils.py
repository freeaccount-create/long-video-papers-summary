...
class Conversation(EasyDict):
    ...
    def get_prompt(self):
        ...
        sep = dict(zip(self.roles, sep))    # {'ASSISTANT:" : '</s>', 'USER:' : ''}
    ...
    def user_query(self, query=None, pre_query_prompt=None, post_query_prompt=None, is_mm=False, num_mm_token=1):
        ...
        role = self.roles[0]    # 'USER: '
        # TODO: remove the num_mm_token and hack the self.mm_token outside
        if is_mm:
            mm_str = num_mm_token*self.mm_token[:-1] + self.mm_token[-1]    # <image>\n
...
conv_eval_vcg_internvl = Conversation(
    system="You are Video-ChatGPT, a large vision-language assistant. "
           "You are able to understand the video content that the user provides, and assist the user with a variety of tasks using natural language."
           "Follow the instructions carefully and explain your answers in detail based on the provided video.",
    roles=("USER: ", "ASSISTANT:"),
    messages=[],
    sep=[" ","</s>"],
    mm_token=f'{INTERNVL_IMG_START_TOKEN}{INTERNVL_IMG_CONTEXT_TOKEN}{INTERNVL_IMG_END_TOKEN}\n',
    mm_style=MultiModalConvStyle.MM_ALONE,
)
...
conv_eval_mvbench_internvl = Conversation(
    system=SYSTEM_MVBENCH,
    roles=("USER: ", "ASSISTANT:"),
    messages=[],
    sep=[" ","</s>"],
...
conv_eval_mvbench_phi3 = Conversation(  # internvl-4B
    system=SYSTEM_MVBENCH,
    roles=['<|user|>\n', '<|assistant|>\n'],
    messages=[],
    sep=[" ",'<|end|>'],
...
conv_eval_mvbench_open_ended = Conversation(
    system="Carefully watch the video and pay attention to the cause and sequence of events, "
           "the detail and movement of objects, and the action and pose of persons. "
           "Based on your observations, answer the question concisely.\n",
    roles=("USER: ", "ASSISTANT:"),
    messages=[],
    sep=[" ","</s>"],
    mm_token='<image>\n',
...
conv_eval_mvbench_open_ended_phi3 = Conversation(  # internvl-4B
    system="Carefully watch the video and pay attention to the cause and sequence of events, "
           "the detail and movement of objects, and the action and pose of persons. "
           "Based on your observations, answer the question concisely.\n",
    roles=['<|user|>\n', '<|assistant|>\n'],
...
conv_eval_gqa = Conversation(
    ...
    sep=[" ","</s>"],
    mm_token='<image>\n',
    mm_style=MultiModalConvStyle.MM_INTERLEAF,  # no "USER: " between vision tokens and question
)
...
conv_eval_videoqabench_internvl = Conversation(
    system="",
    roles=("USER: ", "ASSISTANT:"),
    messages=[],
    sep=[" ","</s>"],
...
conv_templates = {
    ...
    "eval_vcgbench_internvl": conv_eval_vcg_internvl,
    "eval_mvbench": conv_eval_mvbench,
    "eval_mvbench_llavanext": conv_eval_mvbench_llavanext,
    "eval_mvbench_internvl": conv_eval_mvbench_internvl,    # internvl-2b
    "eval_mvbench_phi3": conv_eval_mvbench_phi3,            # internvl-4b
    "eval_videoqabench": conv_eval_videoqabench,
    "eval_videoqa_llavanext": conv_eval_videoqa_llavanext,
    "eval_videoqabench_internvl": conv_eval_videoqabench_internvl,
    "eval_recaption": conv_eval_recaption,
    "eval_recaption_llavanext": conv_eval_recaption_llavanext,
    # Custom task
    "eval_mvbench_open_ended": conv_eval_mvbench_open_ended,
    "eval_mvbench_open_ended_phi3": conv_eval_mvbench_open_ended_phi3,
    "eval_gqa": conv_eval_gqa,
}
...
class EvalDataset(Dataset):
    ...
    def read_video(self, video_path, bound=None, return_index=False):
        # vr = VideoReader(video_path, ctx=cpu(0), num_threads=4)
        # Using num_threads>2 for .webm file raises error: see https://github.com/magic-research/PLLaVA/issues/73 or https://github.com/dmlc/decord/issues/246
    ...
    def set_rank_and_world_size(self, rank, world_size):
        ...
        if self.test_ratio is None or self.test_ratio == 1:
