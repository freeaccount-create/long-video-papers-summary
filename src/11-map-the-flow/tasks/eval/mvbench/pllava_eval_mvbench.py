...
from tasks.eval.model_utils import load_pllava, load_internvl, pllava_answer
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='List of LoRA target modules. In CLI, separate with +. (e.g., "q_proj+v_proj")'
    )
    parser.add_argument(
        "--weight_dir",
        type=str,
...
def load_model_and_dataset(rank, world_size, pretrained_model_name_or_path, num_frames, use_lora, lora_alpha,
                           weight_dir, pooling_shape=(16,12,12), is_tvbench=False, lora_target_modules=('q_proj', 'v_proj')):
    ...
    dataset_class = TVBenchDataset if is_tvbench else MVBenchDataset
    dataset = dataset_class(num_segments=num_frames)
...
def infer_mvbench(
        ...
        print_res=False,
        is_mm=True  # If false, do not use <image> token at all
    ):
    video_list = data_sample["video_pils"]  # list(frame_length) of PIL Image
    conv = conv_templates[conv_mode].copy()

    # append to conv.messages: [['USER:', '<image>\n'], ['USER:', 'Question: ...']
    conv.user_query(data_sample['question'], pre_query_prompt, post_query_prompt, is_mm=is_mm)

    # append to conv.messages: ['ASSISTANT:', 'Best option:(']
...
def run(rank, args, world_size):
    ...
    pooling_shape = args.pooling_shape
