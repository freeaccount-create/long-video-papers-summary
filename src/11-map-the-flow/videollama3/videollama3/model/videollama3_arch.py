...
class Videollama3MetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        grid_sizes: Optional[torch.LongTensor] = None,  # [[4, 20, 14]]
        merge_sizes: Optional[torch.LongTensor] = None, # [2]
        modals: Optional[List[str]] = None, # ['video']
    ):
        ...
        batched_num_patches = grid_sizes.prod(dim=1).div(merge_sizes ** 2).long()   # [280]
        mm_features = self.encode_images(pixel_values, grid_sizes, merge_sizes).to(input_ids.device)    # (280, 3584)
        mm_features = self._get_valid_visual_tokens(mm_features, batched_num_patches, modals)    # (280, 3584)
