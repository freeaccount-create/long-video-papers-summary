...
class PllavaProcessor(ProcessorMixin):
    ...
    def resize_crop_longshort(self, videos: list[list[np.ndarray]], input_data_format):
        ...
        ]   # Fit shortest edge to $shortest_edge while keeping the original aspect ratio e.g., (270, 480) -> (336, 597)
        out_videos = []
        for frames in videos:
            out_frames = []
    ...
    def __call__(
        ...
        if images is not None:
            ...
            pixel_values_list = []
            videos = [[to_numpy_array(image) for image in make_list_of_images(images)] for images in videos]    # list(num_videos) of list(num_frames) of ndarray(h, w, c)
            # images = [self.resize(image, ) if min(get_image_size(image, input_data_format)) < clip_resolution else image for image in images]
            input_data_format = infer_channel_dimension_format(videos[0][0])    # channels_last
            videos = self.resize_crop_longshort(videos, input_data_format)  # All frames are resized to fit shortest edge to desired length (e.g., 336) while maintaining the aspect ratio (e.g., (227, 480) -> (336, 597)
            ...
            for images in videos:
                ...
                pixel_values = self.image_processor(images, return_tensors='np')["pixel_values"]    # resize - centor_crop - rescale - normalize
    ...
    def preprocess_masks(
        ...
        if masks is not None:
            if isinstance(masks, list) and isinstance(masks[0], PIL.Image.Image):
                videos = [masks]  # one video
            else:
                videos = masks

            mask_values_list = []
            videos = [[to_numpy_array(image) for image in make_list_of_images(images)] for images in
                      videos]  # list(num_videos) of list(num_frames) of ndarray(h, w, c)
            # images = [self.resize(image, ) if min(get_image_size(image, input_data_format)) < clip_resolution else image for image in images]
            input_data_format = infer_channel_dimension_format(videos[0][0])  # channels_last
            videos = self.resize_crop_longshort(videos,
                                                input_data_format)  # All frames are resized to fit shortest edge to desired length (e.g., 336) while maintaining the aspect ratio (e.g., (227, 480) -> (336, 597)
            ...
            for images in videos:
                ...
                mask_values = self.image_processor(images, return_tensors='np', do_normalize=False)[
                    "pixel_values"]  # resize - centor_crop - rescale - normalize
                mask_values_list.append(mask_values)

            mask_values = np.concatenate(mask_values_list)
            data.update(mask_values=mask_values)
